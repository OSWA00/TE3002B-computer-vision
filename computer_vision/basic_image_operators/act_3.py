...
def detect_lines(frame):
    ...
    try:
        for index, points in enumerate(linesP):
            lin = linesP[index][0]
            cv.line(frame,(lin[0],lin[1]),(lin[2], lin[3]), (0,255,0), 3, cv.LINE_AA)
    except:
...
def get_blobs(frame):
    ...
    im_with_keypoints = cv.drawKeypoints(frame_gray, keypoints, np.array([]), (0,0,255), cv.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
